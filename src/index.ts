...
import { fetchSudoku } from './fetchSudoku';
import { readSudoku } from './readSudoku';
...
`,
    ...
    if ((!onlineMode && !filename) || (onlineMode && filename)) {
        throw new Error(
            `expected either online mode or a filename to retrieve the sudoku`,
        );
    }
    ...
    const puzzle = onlineMode
        ? await fetchSudoku(difficulty, 'cheon')
        : readSudoku(filename);
    const sudoku = Sudoku.create(puzzle);
