...
class MatrixValidator {
    ...
    matrixIsValid(matrix, diagonal = false) {
        ...
        if (diagonal) {
            ...
            const rowB = [];
            for (let i = 0; i < matrix.length; i++) {
                rowA.push(matrix.get(new Coord_1.Coord(i, i)));
                let inverseI = matrix.length - i - 1;
                rowB.push(matrix.get(new Coord_1.Coord(inverseI, inverseI)));
            }
            if (!this.rowIsValid(rowA))
                return false;
            if (!this.rowIsValid(rowB))
                return false;
        }
    ...
    checkRow(row) {
        ...
        for (const i of row) {
            ...
            if (has.includes(i))
                return false;
            if (i > row.length)
                throw new RangeError(`value '${i}' exceeds row length: '${row.length}'`);
            has.push(i);
        }
        return true;
    }
     ...
     */
    checkMatrix(matrix, diagonal = false) {
        for (let i = 0; i < matrix.length; i++) {
            // check rows left to right
            const row = matrix.getHorizontalRow(i);
        ...
        if (diagonal) {
            ...
            if (!this.checkRow(rowA))
                return false;
            if (!this.checkRow(rowB))
                return false;
        }
        // create a row top to bottom
        for (let i = 0; i < matrix.length; i++) {
